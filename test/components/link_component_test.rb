...
class LinkComponentTest < ViewComponent::TestCase
  ...
  def test_internal_link_opens_in_new_tab_without_icon
    render_inline(LinkComponent.new(label: "Privacy", url: "/privacy"))
    assert_selector "a[target='_blank'][rel='noopener noreferrer']"
  ...
  def test_new_tab_false_stays_in_same_tab_with_no_announcement
    render_inline(LinkComponent.new(label: "Sign in", url: "/accounts/sign-in", new_tab: false))
  ...
  def test_external_override_suppresses_icon_for_absolute_url
    ...
    render_inline(LinkComponent.new(label: "Sign in", url: "https://app.example.com/accounts/sign-in", external: false))
    assert_selector "a[target='_blank']"
    assert_no_selector "svg[role='img']"
    assert_selector "span.sr-only", text: "Opens in a new tab"
  end

  def test_screen_reader_labels_can_be_overridden
    render_inline(LinkComponent.new(label: "Visit us", url: "https://example.com",
      sr_label: "Custom same-tab label", external_sr_label: "Custom external label"))
    assert_selector "span.sr-only", text: "Custom external label"
  end
end
